...
// Multi-select trigger — values carry both a value (identifier) and a label
// (display text). onRemove receives the value string so state removal is direct.
        ...
        {values.length > 0 ? (
          values.map(({ value, label }) => (
            <span key={value}
              ...
              {label}
              <X className="h-3 w-3 cursor-pointer stroke-[3] hover:opacity-70"
...
// Wraps PopoverContent + Command. Defaults to align="start" so the dropdown
// left-aligns with the trigger.
...
export {
  Combobox, ComboboxTrigger, ComboboxMultiTrigger, ComboboxContent,
  CommandInput as ComboboxInput, CommandList as ComboboxList,
  CommandEmpty as ComboboxEmpty, CommandGroup as ComboboxGroup,
  CommandItem as ComboboxItem, CommandSeparator as ComboboxSeparator,
}`
